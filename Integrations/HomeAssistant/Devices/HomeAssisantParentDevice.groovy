...
* 0.1.12 2021-02-08 Dan Ogorchock      Fixed typo in logInfo statement
...
def logsOff(){
    ...
    device.updateSetting("isDebugLogging",[value:"false",type:"bool"])
}

def updated(){
    logInfo("updated...")
    logWarn("debug logging is: ${isDebugLogging == true}")
    logWarn("description logging is: ${isInfoLogging == true}")
    unschedule()
    //if (isDebugLogging) runIn(1800,logsOff)
    initialize()
}
...
def webSocketStatus(String status){
    ...
    if ((status == "status: closing") && (state.wasExpectedClose)) {
        ...
        logWarn "WebSocket error, reconnecting."
        sendEvent(name: "Connection", value: "Reconnecting")
        reconnectWebSocket()
    }
}

def String currentValue(String propertyName)
{ 
    return "unknown"
}
...
def parse(String description) {
    ...
    try{
        ...
        logDebug "parse: domain: ${domain}, device_class: ${device_class}, entity: ${entity}, newVals: ${newVals}, friendly: ${friendly}"
        ...
        switch (domain) {
            case "fan":
                ...
                if (!(choices.contains(speed)))
                    {
                    logDebug "Invalid fan speed received - ${speed}"
                    speed = null
                    }
                ...
                switch (percentage.toInteger()) {
                    ...
                    case 100: 
                        speed = "high"
                    default:
                        logDebug "Invalid fan percentage received - ${percentage}"
                }
                newVals += speed
                newVals += percentage
            ...
            case "climate":
                ...
                break
            default:
                logDebug "No mapping exists for domain: ${domain}, device_class: ${device_class}.  Please contact devs to have this added."
        }
        return
    }  
    catch(e) {
...
def translateDevices(domain, newVals, friendly, origin)
{
    def mapping =
        [
            ...
            button: [type: "Generic Component Button Controller",       event: [[name: "switch", value: newVals[0], type: origin, descriptionText:"${friendly} was turned ${newVals[0]} [${origin}]"]]],
...
def updateChildDevice(mapping, entity, friendly) {
    ...
    if (!ch) {
        logWarn "Child type: ${mapping.type} not created for entity: ${entity}"
        return
    }
    else {
...
def componentPress(ch){
     logDebug("parent.componentPress: ${ch.label}")
...
def componentOn(ch){
    logDebug("received on request from ${ch.label}")
...
def componentOff(ch){
    logDebug("received off request from ${ch.label}")
...
def componentSetLevel(ch, level, transition=1){
    logDebug("received setLevel request from ${ch.label}")
    ...
    if (ch.currentValue("speed")) {
        switch (level.toInteger()) {
            ...
            break
            default:
                logDebug "No case defined for Fan setLevel(${level})"
        }
    } 
    else {        
...
{
    logDebug("received setColor request from ${ch.label}")
...
def componentSetColorTemperature(ch, colortemperature, level, transition=1)
    {
    logDebug("received setColorTemperature request from ${ch.label}")
...
def componentSetHue(ch, hue, transition=1)
    {
    logDebug("received setHue request from ${ch.label}")
...
def componentSetSaturation(ch, saturation, transition=1)
    {
    logDebug("received setSaturation request from ${ch.label}")
...
def componentSetEffect(ch, effectNumber)
    {
    logDebug("received setEffect request from ${ch.label}")
...
def componentSetNextEffect(ch) {
    logWarn "setNextEffect not implemented"
}

def componentSetPreviousEffect(ch) {
    logWarn "setPreviousEffect not implemented"
}

def componentSetSpeed(ch, speed) {
    logDebug("received setSpeed request from ${ch.label}, with speed = ${speed}")
    int percentage = 0
    switch (speed) {
        case "on":
            data = [:]
        ...
        case "high":
            ...
            break
        default:
            logDebug "No case defined for Fan setSpeed(${speed})"
    }
}
...
void operateCover(ch, op) {
    logDebug("received ${op} request from ${ch.label}")
...
def componentRefresh(ch){
    logDebug("received refresh request from ${ch.label}")
...
def componentSetCoolingSetpoint(ch, temperature){
    logDebug("received setCoolingSetpoint request from ${ch.label}")
    
    tmode = ch.currentValue("thermostatMode")
    logDebug("thermostatMode is ${tmode}")
...
def componentSetHeatingSetpoint(ch, temperature) {
    logDebug("received setHeatingSetpoint request from ${ch.label}")
...
def componentSetThermostatFanMode(ch, fanmode) {
    logDebug("received fanmode request from ${ch.label}")
...
{
    logWarn ("Start level change not supported")
}

def componentStopLevelChange(ch)
{
    logWarn ("Stop level change not supported")
}
...
{   
    logDebug("parent.executeCommand: ${ch.label}")  
    logDebug("parent.executeCommand(ch: ${ch}, service: ${service}, data: ${data}")  
    entity = ch?.name
    ...
    logDebug("messUpdStr = ${messUpdStr}")
    interfaces.webSocket.sendMessage(messUpdStr)    
}

def deleteAllChildDevices() {
    logInfo "Uninstalling all Child Devices"
